...
// Define menu types
type MenuType = "construction" | "robots" | "none";
...
export class ActionMenu {
  ...
  public toggleMenu(menuType: MenuType, forceState?: boolean): void {
    // If a state is forced, use that, otherwise toggle
    const shouldOpen =
      forceState !== undefined ? forceState : this.activeMenu !== menuType;

    // Close all menus first
    this.closeAllMenus();

    // If we're closing the menu or switching to a different one, we're done
    if (!shouldOpen) {
      return;
    }

    // Close bulldoze mode if opening a menu
    if (this.isBulldozeModeActive) {
      this.toggleBulldozeMode(false);
    }
  ...
  private resetButtonHighlights(): void {
    [this.buildButton, this.robotsButton].forEach((button) => {
      const buttonBg = button.getAt(1) as Phaser.GameObjects.Rectangle;
      const buttonBorder = button.getAt(0) as Phaser.GameObjects.Rectangle;
      const buttonText = button.getAt(2) as Phaser.GameObjects.Text;
  ...
  // Public getter for the active menu
